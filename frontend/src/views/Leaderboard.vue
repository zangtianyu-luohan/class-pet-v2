    ...
    <div v-else class="rank-list">
      <div v-for="item in data" :key="item.rank" class="rank-card" :class="{ 'top3': item.rank <= 3 }">
        ...
        <div class="rank-info">
          ...
          <div class="rank-meta">积分 {{ item.points }}</div>
        </div>
...
async function fetchData() {
  ...
  loading.value = true
  try {
    const endpoint = `/api/leaderboard/${activeTab.value}`
...
.podium-name { font-weight: 600; color: #1e293b; font-size: 14px; margin: 4px 0; }
