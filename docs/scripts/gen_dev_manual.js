...
function coverPage() {
  return [
    ...
    new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 600 }, children: [new TextRun({ text: "（含 EXE 桌面版构建指南）", font: "Microsoft YaHei", size: 24, color: "888888" })] }),
    ...
    new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "技术栈：Vue 3 + FastAPI + SQLite", font: "Microsoft YaHei", size: 20, color: "999999" })] }),
    pb(),
  ];
}
  ...
  sections: [{
    ...
    children: [
      ...
      makeTable(["工具", "版本要求", "用途"], [
        ...
        ["Python", "≥ 3.11", "后端运行"],
        ["SQLite", "3.x", "数据库（零配置，随 Python 自带）"],
      ], [2500, 2000, 4526]),
      h2("2.2 本地启动"),
      h3("后端"),
      b("cd backend"),
      ...
      h1("第5章 构建与部署"),
      h2("5.1 EXE 桌面版构建"),
      p("使用 PyInstaller 将后端打包为独立 EXE，用户双击即可运行："),
      b("# 1. 构建前端"),
      ...
      b(""),
      b("# 3. 运行构建脚本"),
      b("cd build_exe && build.bat"),
      p("构建产物输出到 build_exe/dist/学生积分管理系统/ 目录。"),
      h2("5.2 源码直接运行"),
      p("开发或调试时可直接运行源码："),
      b("# 后端"),
      b("cd backend && pip install -r requirements.txt"),
      b("uvicorn app.main:app --reload --port 8000"),
      b(""),
      b("# 前端（另开终端）"),
      ...
      makeTable(["问题", "解决方案"], [
        ...
        ["PyInstaller 打包缺少模块", "使用 --hidden-import 显式指定所有依赖"],
      ], [4000, 5026]),
      pb(),
      ...
      makeTable(["用途", "用户名", "密码", "地址"], [
        ["教师端", "teacher", "123456", "http://127.0.0.1:8866"],
        ["管理后台", "admin", "Admin123456", "http://127.0.0.1:8866/admin-panel"],
      ], [2000, 2000, 2526, 2500]),
    ]
  }]
});
