...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      p("• 每条记录显示：学生姓名、积分变动（+/-）、原因、类型、时间"),
      p("• 支持按时间倒序查看"),
      h2("8.2 筛选与搜索"),
      p("• 关键词搜索：输入学生姓名或原因关键字，快速定位记录"),
      p("• 类型筛选：按积分类型筛选（手动调整/奖励/惩罚/抽奖中奖/抽奖消耗/积分兑换）"),
      p("• 日期范围：选择开始和结束日期，查看指定时间段的积分变动"),
      h2("8.3 导出 Excel"),
      p("点击页面右上角「📥 导出Excel」按钮，可将当前筛选结果导出为 Excel 文件。"),
      p("导出内容包括：学生姓名、积分变动、原因、类型、时间。"),
      h2("8.4 使用场景"),
      p("• 课堂结束后回顾积分变动"),
      p("• 家长会时展示学生积分情况"),
      p("• 核对积分异常变动"),
      ...
      p("• 创建新用户、删除用户"),
      p("• 重置用户密码"),
      p("• 设置账号有效期：点击「设有效期」按钮，支持 +30天、+90天、+1年、永久等快捷选项"),
      p("• 账号过期后显示红色标记，未过期显示绿色标记"),
      p("• 过期账号无法登录，已登录的用户也会在操作时被强制退出"),
      h2("10.4 数据库管理"),
      p("• 查看所有数据表，浏览表数据"),
      p("• 支持新增、编辑、删除记录"),
      p("• 支持所有字段类型的编辑（文本、数字、布尔、日期时间等）"),
      h2("10.5 登录日志"),
      p("• 查看所有登录记录，筛选成功/失败记录"),
      p("• 用于安全审计和异常登录排查"),
      h2("10.6 积分日志"),
      p("• 查看所有积分变动记录"),
      p("• 按学生、类型、时间筛选"),
      h2("10.7 数据备份"),
      p("• 一键导出完整数据库备份（JSON 格式）"),
      p("• 用于数据迁移和灾难恢复"),
      pb(),
      ...
      makeTable(["问题", "解答"], [
        ...
        ["正在使用时突然跳到登录页", "账号有效期已到期，系统会自动强制退出当前会话。联系管理员续期后重新登录。"],
        ["积分提醒自动弹窗怎么关", "在课堂工具 → 积分提醒中，关闭「进入系统时自动弹窗提醒」开关即可。"],
      ], [4000, 5026]),
